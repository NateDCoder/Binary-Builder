...
__all__ = ["NathanAdam", "adam"]


class NathanAdam(Optimizer):
    def __init__(
        self,
        params: ParamsT,
    ...
    *,
    ...
    for (
        ...
        for i in range(len(device_params)):
            ...
            old_average = grad.abs().mean()
            # Create masks where we want to zero the gradients
            
            too_high = (param > 4.9) & (grad < 0)
            too_low = (param < -1.9) & (grad > 0)

            mask = ~(too_high | too_low)  # Keep gradients where neither condition is true

            new_grad = grad * mask
            
            new_average = new_grad.abs().mean()
            if new_average == 0:
                continue
            # Apply the mask
            device_grads[i] = new_grad * old_average / new_average  # Zero out the undesired gradient elements
        ...
        for i in range(len(device_params)):
            delta = step_size[i] * (device_exp_avgs[i] / exp_avg_sq_sqrt[i])
            average_deltas.append(float(torch.abs(delta).mean()))
